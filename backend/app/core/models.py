...
class SystemReading(BaseModel):
    ...
    wind_speed: float = Field(default=0.0, ge=0, description="Wind speed in m/s (manual input)")
    rpm: float = Field(default=0.0, ge=0, description="Propeller RPM from Arduino")
    lift_force: float = Field(default=0.0, description="Lift/Sustentation force in N from Arduino")
...
class WindSpeedCommand(BaseModel):
    """
    Command received from frontend to update wind speed.
    """
    wind_speed: float = Field(ge=0, description="Wind speed value in m/s")
