...
class MeasurementManager:
    ...
    def __init__(
        ...
        # State
        self._current_wind_speed: float = 0.0
        ...
        # Lock for thread-safe wind speed updates
        self._lock = asyncio.Lock()
    
    @property
    def current_wind_speed(self) -> float:
        """Get the current wind speed value."""
        return self._current_wind_speed
    
    async def set_wind_speed(self, speed: float) -> None:
        """
        Update the wind speed from frontend input.
        Thread-safe operation.
        """
        async with self._lock:
            self._current_wind_speed = max(0, speed)
            logger.debug(f"Wind speed updated to: {self._current_wind_speed} m/s")
        ...
        while self._is_running:
            try:
                ...
                if not self.sensor.is_connected():
                    logger.info(f"Sensor disconnected, retrying in {reconnect_interval}s...")
                ...
                if arduino_reading and arduino_reading.is_valid:
                    # Fuse data: Arduino reading + current wind speed + timestamp
                    async with self._lock:
                        wind_speed = self._current_wind_speed
    ...
    def get_status(self) -> SystemStatus:
        ...
        return SystemStatus(
            ...
            current_wind_speed=self._current_wind_speed,
