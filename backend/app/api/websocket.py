...
from app.core.models import WindSpeedCommand, SystemReading
from app.services.connection_manager import connection_manager
from app.api.dependencies import get_measurement_manager
...
async def websocket_endpoint(
    ...
    Receives:
    - Wind speed updates: {"type": "wind_speed", "value": 10.5}
    - Control commands: {"type": "command", "action": "start_recording"|"stop_recording"|"clear"}
    
    Sends:
    - System readings: {"timestamp": "...", "wind_speed": 10.5, "rpm": 1500, "lift_force": 2.5}
    ...
    try:
        while True:
            ...
            try:
                ...
                if msg_type == "wind_speed":
                    # Update wind speed
                    value = float(message.get("value", 0))
                    await measurement_manager.set_wind_speed(value)
                    logger.info(f"Wind speed set to: {value} m/s")
                    
                    # Confirm to client
                    await connection_manager.send_personal(websocket, {
                        "type": "wind_speed_confirmed",
                        "value": value
                    })
